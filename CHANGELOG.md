...
- **Модерация** — fallback превью при ошибке VK; настройки хранилища и конфигурации превью.
- **Авторизация** — поле `is_admin` в ответе `GET /api/auth/me`.
- **Битва** — приём `session_token` в JSON теле голоса для анонимных пользователей за прокси; in-memory Redis `SET NX`, демо-fallback пары.

### Синхронизация с фронтом

- Релиз фронтенда **v2.1.0** (см. репозиторий `magikbook`): submit/upload, админ-модерация, превью изображений, битва.

---

## [2025-03-23] - Real Battle Voting System

### Исправлено (Backend)

#### Battle Voting System (Работает по-настоящему!)
- **Таблица `battle_votes`** — сохраняет каждый голос с привязкой к пользователю/сессии
- **Защита от накрутки** — проверка `uq_battle_vote_user_pair` (один голос на пару)
- **Реальные проценты** — рассчитываются на основе всех голосов в базе
- **API Changes**:
  - `POST /api/battle/vote` — теперь возвращает `{left_pct, right_pct, total_votes}`
  - `GET /api/battle/stats/{prompt_id}` — получить статистику конкретного промпта
- **ELO рейтинг** — обновляется при каждом голосовании (как и раньше)

### Исправлено (Frontend)
- **BattleArena** — показывает реальное количество голосов и проценты
- **Session token** — анонимные пользователи тоже могут голосовать
- **Обработка ошибок** — дубликаты голосов (409), требование авторизации (401)
...
- **Мульти-модельный fallback** — автоматический переключение при rate limit:
  - Приоритет:
    1. `gemini-2.5-flash-lite-preview-06-17` — новая быстрая и дешевая (2.5 поколение)
    2. `gemini-2.5-flash-preview-04-17` — 2.5 поколение, улучшенная версия
    3. `gemini-2.0-flash-lite` — базовая дешевая модель
    4. `gemini-2.0-flash` — стандартная 2.0
    5. `gemini-1.5-flash` — запасная предыдущего поколения
  - Автоматический retry при 429 Too Many Requests
  - Graceful degradation с информативным сообщением пользователю
